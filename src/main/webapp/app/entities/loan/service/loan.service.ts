...
type RestOf<T extends ILoan | NewLoan> = Omit<T, 'startDate' | 'endDate'> & {
...
export class LoanService {
  ...
  protected convertDateFromClient<T extends ILoan | NewLoan | PartialUpdateLoan>(loan: T): RestOf<T> {
    return {
      ...loan,
      startDate: loan.startDate?.format(DATE_FORMAT) ?? null,
      endDate: loan.endDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restLoan: RestLoan): ILoan {
    return {
      ...restLoan,
      startDate: restLoan.startDate ? dayjs(restLoan.startDate) : undefined,
      endDate: restLoan.endDate ? dayjs(restLoan.endDate) : undefined,
    };
  }
